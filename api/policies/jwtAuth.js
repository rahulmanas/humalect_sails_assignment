...
const { customToJSON } = require("../models/Auth");
...
module.exports = function (req, res, next) {
  ...
  if (!token) {
    return res.badRequest(
      Utils.jsonErr('Format is "Authorization: Bearer [token]"')
    );
  }

  UserManager.authenticateUserByToken(token)
    .then((user) => {
      req.userInfo = customToJSON(user);
      next();
    })
    .catch((err) => {
      switch (err) {
